...
import 'personal_info_screen.dart';
import 'GetHelpScreen.dart';
// NEW: import for full history page
import 'full_points_history_screen.dart'; // NEW
...
class _WelcomeScreenState extends State<WelcomeScreen> {
          ...
          children: [
            Padding(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  borderRadius: BorderRadius.circular(16),
                  border: Border.all(width: 4, color: Colors.grey.shade700),
                ),
                padding: const EdgeInsets.all(2.5),
                child: Container(
                  decoration: BoxDecoration(
  ...
  // CHANGE: accept optional maxItems to limit how many we render on Home
  Widget _buildHistoryList({int? maxItems}) { // CHANGE
    if (_isLoadingHistory) {
      return const Padding(
        padding: EdgeInsets.all(16),
        child: CircularProgressIndicator(),
    ...
    // NEW: choose the slice to show
    final items = (maxItems != null && _pointHistory.length > maxItems)
        ? _pointHistory.take(maxItems).toList()
        : _pointHistory; // NEW
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                children: [
                  const SizedBox(height: 8),
                  _buildPointsCard(context, _points),
                  // CHANGE: show history section (max 3) + "View more" button
                  if (_showHistory || _isLoadingHistory) ...[ // CHANGE
                    _buildHistoryList(maxItems: 2), // CHANGE
                    if (!_isLoadingHistory && _pointHistory.length > 2) // NEW
                      Align( // NEW
                        alignment: Alignment.centerRight, // NEW
                        child: TextButton( // NEW
                          onPressed: () {
                            Navigator.push(
                              context,
                              MaterialPageRoute(
                                builder: (_) => const FullPointsHistoryScreen(), // NEW
                              ),
                            );
                          },
                          child: const Text('View more'), // NEW
                        ),
                      ),
                  ],
                  const SizedBox(height: 12),
